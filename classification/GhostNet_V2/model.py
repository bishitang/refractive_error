...
class GhostNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=2, width=1.0, dropout=0.2, block=GhostBottleneckV2, args=None):
...
def ghostnetv2(num_classes=2, width=1.0, dropout=0.2, args=None):
    cfgs = [
        # k, t, c, SE, s
        [[3, 16, 16, 0, 1]],
        [[3, 48, 24, 0, 2]],
    ...
    return GhostNetV2(cfgs=cfgs, num_classes=num_classes, width=width, dropout=dropout, args=args)


if __name__=='__main__':
    model = ghostnetv2(num_classes=7, width=1.0, dropout=0.2, args=None)
    model.eval()
    print(model)
    input = torch.randn(2,18,80,80)
    y = model(input)
    print(y.size())