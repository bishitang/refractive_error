...
class Trainer:
    def __init__(self, path, model, model_copy, img_save_path):
        ...
        self.val_dataset = DataLoader(datasets.Datasets(valset_list, path, False), batch_size=16, shuffle=False, num_workers=0)
        ...
        for epoch in range(1, stop_value + 1):
            ...
            if epoch % 1 == 0 or epoch == stop_value:
                ...
                with torch.no_grad():
                    ...
                    # 保存准确率最高的三个模型
                    save_path_acc = r'D:\shishai\model\resnet\params/resnet_val_acc_{:.3f}_{:.3f}_epoch{}.plt'.format(
