...
class Tester:
    ...
    def test(self, img, txt):
        ...
        if abs(txt[0] - out[0]) <= 5:
            AX = 1
        elif abs(txt[0] - out[0]) <= 10:
            AX = 2
        elif abs(txt[0] - out[0]) <= 15:
            AX = 3
        elif abs(txt[0] - out[0]) <= 20:
            AX = 4
        elif abs(txt[0] - out[0]) <= 25:
            AX = 5
        else:
            AX = 0
...
if __name__ == '__main__':
    ...
    # 路径改一下
    t = Tester(r'D:\shishai\model\github\refractive_error\axis_prediction\params_resnet\resnet_axis_val_acc_0.587_53.991_epoch10.plt', r'./model_{}_{}.plt')
